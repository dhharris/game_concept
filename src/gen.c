...
float *gen_perlin_noise(int width, int height)
{
...
Vector2 get_stairs_coord(float *perlin_noise)
{
        ...
        int rand_y = arc4random() % (LEVEL_SIZE - 2) + 1;
        while (perlin_noise[rand_x * LEVEL_SIZE + rand_y] < NOISE_THRESHOLD) {
                rand_x = arc4random() % (LEVEL_SIZE - 2) + 1;
                rand_y = arc4random() % (LEVEL_SIZE - 2) + 1;
        }
...
void gen_level()
{
        ...
        Vector2 stair_down = get_stairs_coord(perlin_noise);
                ...
                for (x = 0; x < LEVEL_SIZE; ++x) {
                        ...
                        if (Vector2Eq(stair_up, curr))
                                ...
                                fprintf(f, ">");
                                 ...
                                 x == 0 || y == 0)
                                fprintf(f, "#");
                        else if (perlin_noise[x * LEVEL_SIZE + y] >=
                                 NOISE_THRESHOLD)
                                fprintf(f, " ");
                        else
                                fprintf(f, "#");
